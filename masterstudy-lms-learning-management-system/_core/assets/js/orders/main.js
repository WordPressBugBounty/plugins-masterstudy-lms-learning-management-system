...
(function ($) {
  ...
  var getBrowserLocale = function getBrowserLocale(wpLocale) {
    var normalizedLocale = (wpLocale || '').replace(/_/g, '-');
    var _normalizedLocale$spl = normalizedLocale.split('-'),
      _normalizedLocale$spl2 = _slicedToArray(_normalizedLocale$spl, 2),
      languageCode = _normalizedLocale$spl2[0],
      regionCode = _normalizedLocale$spl2[1];
    var candidates = [normalizedLocale, languageCode && regionCode ? "".concat(languageCode, "-").concat(regionCode) : '', languageCode].filter(function (locale, index, locales) {
      return locale && locales.indexOf(locale) === index;
    });
    ...
    try {
      for (_iterator.s(); !(_step = _iterator.n()).done;) {
        ...
        if (!window.Intl || typeof Intl.getCanonicalLocales !== 'function') {
          return locale;
        }
        try {
  ...
  function buildOrderTimeline(order, item) {
    try {
      ...
      for (var i = 1; i <= billingCycles; i++) {
        ...
        var locale = getBrowserLocale(order.locale);
